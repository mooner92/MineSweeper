...
export async function POST(req: Request) {
  ...
  // 지원번호 ("0323-000050 (이형민)" or "0323-000050(이형민).zip").
  ...
  // a bare "배한나" is a name, not an id, and must NOT dedup against other bare names.
  ...
  // (a bare name like "이형민" is unsafe to dedup on — 동명이인).
  if (externalId) {
