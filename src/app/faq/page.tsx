    ...
    >
      <div className="space-y-2">
        ...
        <Faq q="“동명이인/약어”는 무슨 뜻인가요?">
          같은 사람인지 다른 사람인지 자동으로 단정하기 애매한 경우입니다. 예: <strong>정민 / 정민호</strong>
        ...
        <Faq q="구글스칼라(h-지수) 캡처는 공저자가 적게 잡혀요. 왜 그런가요?">
          <strong>구글스칼라 프로필 캡처는 자동 추출이 본질적으로 불완전한 “확인 필수” 예외
          문서</strong>입니다. 화면에서 각 논문의 저자 목록이 <strong>“…”로 잘려</strong> 보이고(그 뒤
          저자는 이미지 안에 아예 없음), 우측 <strong>공저자 패널</strong>도 일부만 보이거나 가려지는
          경우가 많아, 캡처 안에 실제로 담긴 이름 자체가 제한적입니다. 게다가 약어형
          이름(예: <strong>G Kim</strong>)이 많아 정확 매칭도 어렵습니다. 그래서 시스템은 h-지수에서
          뽑은 인물을 <strong>전원 “미확인”</strong>으로 표시해 검토를 강제하니, <strong>반드시 캡처
          원본과 대조</strong>해 주세요. 공저자를 빠짐없이 보려면 논문의 <strong>“전체 공저자(VIEW
          ALL)” 화면</strong>을 캡처하거나, 풀네임 저자가 담긴 <strong>전문(全文) 논문·대표연구실적</strong>을
          함께 올리는 것이 가장 정확합니다.
        </Faq>
      </div>
