...
- **동명이인/약어 판정** = 4단에서 같은 성씨 + **자모(字母) 1자 차이(오인식)** 또는 **통째 음절 접두(약어)** 만 후보로 묶어 사람이 확인(자동병합 금지). `이주영↔이조영`·`김용↔김용표`는 묶고, `김진영↔김진석`(자모 3자 차이)은 안 묶음.
...
> - **지원자 중복 제거**: 같은 **지원번호**(`0323-000050`)의 zip을 다시 올리면 **덮어쓰기**(이전 추출 교체) — 항상 1지원자 = 1카드.
