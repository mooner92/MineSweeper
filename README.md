...
> - **검토 화면 분류**: 관계자 목록을 **관계 유형(지도교수·심사위원·공저자·연구진…)별 그룹 + 좌측 사이드바 체크박스 필터(상태·역할) + 이름·소속 검색 + 문서별 인원 태그**로 정리.
...
> ⚠️ **구글스칼라(h-지수) 캡처 = "확인 필수" 예외 문서.** 구글스칼라 프로필 화면은 각 논문의 저자
> 목록을 `…`로 잘라 보여주고(그 뒤 저자는 이미지에 없음), 우측 공저자 패널도 일부만/가려져 나오는
> 경우가 많아 **캡처 안에 담긴 이름 자체가 제한적**입니다(약어형 이름도 많아 매칭도 어려움). 해상도를
> 올려도 회수율이 늘지 않음을 실측 확인 — 모델 한계가 아니라 **소스 한계**입니다. 따라서 h-지수 추출
> 인물은 **전원 "미확인"으로 강제 표시**해 사람이 원본과 대조하게 하며, 공저자를 빠짐없이 보려면 논문
> "전체 공저자(VIEW ALL)" 화면 캡처 또는 풀네임 저자가 담긴 **전문(全文) 논문·대표연구실적**을 함께
> 올리는 것이 정확합니다.
...
2. **지원자별 검토** — PC 풀블리드 ERP 2단 레이아웃. **좌측 고정 사이드바**: 상태(검토 필요·제척·동일소속)·역할 다중 선택 체크박스 필터(패싯 카운트 포함) + **자동 점검**(본인 식별·검출 커버리지·스캔 문서·검토 대기·동일소속 — 이상 시 경고).
   **메인 영역**: 관계 유형별 그룹 + 문서별 인원 태그, **저신뢰·미확인 = 노랑 "미확인"**(클린 행은 신뢰도 배지 없음), **동명이인/약어 후보 병기**, 확인/수정/제외, **본인 자동 제외**,
   **동일소속 배지**(본인 소속기관과 같은 기관 추정 시), **추출실패 가시화**(텍스트 문서인데 0명 → "0명 — 확인" 배지),
   **전문가 풀 대조**(관계자와 이름이 일치하는 심사위원 후보 = **제척 대상**. 이름 일치 기준이라 동명이인 주의 표시 — **행 클릭 시** 전문가 소속·직위·분야·근거 문서·페이지·이메일 등 상세를 Notion식 토글로 펼침).
   헤더 아래 **번호 단계 안내**(① 수동 확인 → ② 관계자 검토 → ③ 제척 확정 → ④ 면접위원 초빙)로 첫 방문자 동선을 안내. 교정 내역은 `corrections` 에 적재.
3. **검토 필요 큐** — 도장·손글씨·비전판독 필요 항목을 크롭 갤러리로 모아보기. **당근마켓 스타일 좌측 sticky 필터 사이드바**(유형·지원자 다중 선택 체크박스, 카운트 표시, 그룹 내 OR·그룹 간 AND 즉시 필터링, 초기화)로 항목을 빠르게 좁힐 수 있습니다.
...
5. **면접위원 초빙(지원자별)** — 전문가 풀에서 **제척·기담음을 제외한 후보**를 분야(대/중분류)·이름·소속으로 좁혀
...
5-1. **면접위원 섭외(회차 단위)** — `/rounds`. 회차(또는 그 안에서 고른 **서류 합격자**)의 **제척 대상을
   합집합**으로 풀에서 제거하고, **아무에게도 제척되지 않은 전문가**를 면접위원 후보로 제시·내보내기(엑셀 2시트:
   섭외 가능 / 제척+사유). 제척 + 섭외가능 = 풀(충돌 인원이 후보로 새지 않음). 전문가 풀은 **교체 가능한
   외부 명단**으로, 추후 **원장 추천 명단**(같은 포맷)으로 적재 교체하면 그 기준으로 동작합니다.
