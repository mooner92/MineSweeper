...
describe('classifyDocType', () => {
  it('uses the [tag] first', () => {
    expect(classifyDocType({ filename: '0323-000001_[학위논문]_x.pdf' }).docType).toBe('degree_thesis');
    expect(classifyDocType({ filename: '0323-000001_[대표연구실적]_x.pdf' }).docType).toBe(
      'representative_research',
    );
    expect(classifyDocType({ filename: '0323-000001_[학술논문]_x.pdf' }).docType).toBe('journal_article');
  });

  it('detects hindex from the filename', () => {
    const r = classifyDocType({ filename: '0323-000001_hindex.png' });
    expect(r.docType).toBe('hindex');
    expect(r.method).toBe('filename');
  });

  it('falls back to content: thesis markers', () => {
    const r = classifyDocType({ filename: 'scan001.pdf', firstPageText: '석사 학위논문 지도교수 정주철' });
