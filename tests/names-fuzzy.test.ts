...
describe('jamoEditDistance', () => {
  ...
  it('is 3 for two unrelated syllables (박철수 vs 박철민: 영 vs 석)', () => {
    // The whole point: syllable-level distance was 1 (and wrongly flagged); jamo-level is 3.
    expect(jamoEditDistance('박철수', '박철민')).toBe(3);
  });
...
describe('koreanNearDuplicates', () => {
  ...
  it('does NOT flag clearly different given names (박철수 vs 박철민)', () => {
    expect(koreanNearDuplicates('박철수', ['박철민'])).toEqual([]);
    expect(koreanNearDuplicates('김민수', ['김민호'])).toEqual([]); // 2 자모 apart → distinct
  });

  it('flags a whole-syllable prefix as an abbreviation (정민 → 정민호)', () => {
    const r = koreanNearDuplicates('정민', ['정민호']);
    expect(r.map((x) => x.name)).toEqual(['정민호']);
