...
describe('StubExtractor — degree thesis', () => {
  it('parses advisor / committee / department head from the approval page', async () => {
    ...
    expect(byRole('supervisor')).toContain('이준호');
    expect(byRole('committee')).toEqual(expect.arrayContaining(['박서준', '최지훈', '이준호']));
    expect(byRole('department_head')).toContain('윤도현');
...
describe('StubExtractor — article', () => {
  it('parses coauthors and ignores the references section', async () => {
    ...
    expect(names).toEqual(
      expect.arrayContaining(['Gildong Hong', 'John Carter', 'Mark Lee']),
    );
    ...
    expect(persons[0].affiliation).toBe('example.edu');
  });
  ...
  it('tags the applicant themself via conservative matching', async () => {
    ...
    expect(persons.find((p) => p.nameRaw === 'John Carter')?.isSelf).toBe(true);
    expect(persons.find((p) => p.nameRaw === 'Gildong Hong')?.isSelf).toBe(false);
  });
});
...
describe('StubExtractor — title fabrication guard', () => {
  it('does not capture Title-Case paper titles as coauthors', async () => {
    ...
    const names = persons.map((p) => p.nameRaw);
    expect(names).toEqual(expect.arrayContaining(['Emily Park', 'John Carter']));
...
describe('StubExtractor — co-supervisor spacing', () => {
  it('classifies "부 지도교수" as co_supervisor (not supervisor)', async () => {
    const text = ['도시공원 연구', '', '지도교수  이준호', '부 지도교수  김민수', '심사위원  이영희'].join('\n');
    const persons = await stub.extract({
      docType: 'degree_thesis',
      filename: 't.txt',
      pages: [page(text)],
    ...
    expect(persons.find((p) => p.nameRaw === '이준호')?.role).toBe('supervisor');
  });
});
