...
describe('crossCheck', () => {
  it('is a NO-OP for non-thesis docTypes (hindex / journal / representative)', () => {
    const out = crossCheck([
      ...
      person({ nameRaw: 'Galen Newman', docType: 'representative_research', sourceKind: 'printed' }),
    ]);
  ...
  it('confirms a thesis seal that matches a co-located printed anchor', () => {
    const out = crossCheck([
      person({ nameRaw: '정주철', docType: 'degree_thesis', sourceKind: 'printed' }),
      person({ nameRaw: '정주철', docType: 'degree_thesis', sourceKind: 'seal' }),
    ]);
  ...
  it('flags mismatch when the thesis seal name differs from the printed anchor', () => {
    const out = crossCheck([
      person({ nameRaw: '정주철', docType: 'degree_thesis', sourceKind: 'printed' }),
      person({ nameRaw: '김영호', docType: 'degree_thesis', sourceKind: 'seal' }),
    ]);
