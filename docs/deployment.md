...
#### ⚠️ 업로드 크기 한도 (Cloudflare 100MB)

Cloudflare 무료/프로 플랜은 **요청 본문을 약 100MB로 제한**한다. 100MB를 넘는 압축파일은 **edge에서 차단되어
서버에 도달하지 못한다**(서버 로그·업로드 디렉터리에 흔적이 남지 않음 → "업로드 실패"로만 보임). 서버 자체 한도는
`MAX_UPLOAD_BYTES`(기본 200MB)다.

- **해결 1 — 내부망 직접 업로드(권장)**: 같은 망에서 `http://<서버-LAN-IP>:3100`(예: `http://192.168.1.105:3100`)으로
  접속해 업로드하면 Cloudflare를 우회한다(서버 200MB까지). `next start`는 `0.0.0.0:3100`에 바인딩되어 LAN에서 접근 가능.
- **해결 2 — zip 분할**: 카테고리 단위로 나눠 100MB 미만으로 업로드.
- **해결 3 — 한도 상향**: Cloudflare Business(200MB)/Enterprise로 업그레이드(인프라 결정).

`UploadForm`은 공개 도메인에서 100MB 초과 파일을 **업로드 전에 감지**해 위 안내 메시지를 띄운다.
