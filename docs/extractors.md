...
| 이니셜 토큰 | `hasInitial = /(^\|\s)[A-Z]\.?(\s\|$)/` | `John D. Carter` 같은 가운데 이니셜 |
...
4. 그룹 내 표기 분기(`John Carter`/`J Carter`)는 `nameCandidates`로, 표시 이름은 **보고확률 최고치**.
...
| `hindex` | 구글스칼라 캡처 이미지에서 공저자 패널·논문별 저자를 coauthor 로 추출, 약어형 이름(`G Hong`, `J Carter`) 많음 |
