...
## 7b. 멀티모델 앙상블 (구현됨, 라이브 보류) · 도장 가능성 · 웹검색(향후)

### 앙상블 투표 (vLLM, 로컬 전용) — 구현 완료
사용자 요청("모델 3개를 한 번씩 돌려 확률 높은 걸 고르고 불일치는 필터링")을 `EnsembleExtractor`
(`extract/ensemble.ts`)로 구현. 같은 문서를 N개 로컬 vLLM에 보내 `namesMatch`로 그룹핑→**합의비율
(votes/N)=신뢰도**, 그룹 내 표기 분기는 `nameCandidates`(표시는 보고확률 최고치), 근접 오독은 하류
gazetteer(1.5a)가 후보화. `EXTRACTOR_MODE=ensemble` + `VLM_ENSEMBLE`로 활성화. 단위테스트 완비
(`tests/ensemble.test.ts`, 목업). 외부 API 미사용. 자세히는 [extractors.md §3b](./extractors.md).

> **라이브 보류 사유**: 이 서버 2×A40이 공유 vLLM(Qwen2.5-Coder-32B, 타 사용자)으로 가득 차 모델
> 서빙/실측 불가 → GPU 여유 확보가 선결. 스크립트: [serve-ocr.sh](../scripts/serve-ocr.sh) /
> [download-ocr-models.sh](../scripts/download-ocr-models.sh).

### 도장(인장) 감지 가능성 — 평가
- **검출(어디에 도장이 있나)** 은 가능: 범용 VLM/검출기로 도장 영역 박스는 잡을 수 있음.
- **인식(전서체 도장 글자 읽기)** 은 **신뢰 어려움**: 篆刻/곡선 전서체는 사람도 자주 못 읽고, 공개
  벤치/한국어 데이터가 없음. 앙상블을 써도 모델들이 **제각각 다르게 읽어 합의가 안 날 가능성**이 큼
  → 그 자체가 좋은 신호: **합의 실패 → 자동 abstain → 사람**(원하는 "필터링"과 정확히 일치).
- **권고(불변)**: 도장/서명은 **자동확정 금지, 검출→사람**. 앙상블은 "읽혔다고 우기는 단일 모델"의
  오추출을 합의로 걸러내는 용도. **라이브 적합성은 GPU 여유 + 내일 실제 샘플로 확인** 예정(현재는 평가만).

### 웹검색 크로스체크 — 향후 플랜(고려만)
추출 인물명을 웹검색으로 교차검증/소속 보강하는 아이디어. **지금은 구현하지 않음**(사용자 지시: 나중
플랜). 단, 본 앱은 **온프레/외부호출 금지**가 원칙이므로 도입 시 (a) 공공기관 보안 정책상 외부 검색
허용 여부, (b) PII(지원자 이름)를 외부 검색에 보내는 것의 적법성 검토가 **선결**. → Phase 2 소속검색과
함께 별도 결정.
