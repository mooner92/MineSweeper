...
문서에 "정주철 (인)", "홍길동 (서명)" 처럼 날인·서명 표기가 이름에 붙어 들어오는 경우가 잦다.
...
| `정주철 (인)` | `정주철` |
| `홍길동 (서명)` | `홍길동` |
| `Galen Newman (signature)` | `Galen Newman` |
...
한국어 문서는 강조를 위해 이름 글자 사이를 띄우는 경우가 있다("정 주 철"). 이를 "정주철" 로
...
| 입력 | 토큰 | 모두 단음절? | 결과 | 이유 |
| --- | --- | --- | --- | --- |
| `정 주 철` | `정`,`주`,`철` | ✅ | `정주철` | 전부 1글자 → 자간으로 판단, 붙임 |
| `정  주  철` | `정`,`주`,`철` | ✅ | `정주철` | ①에서 다중 공백이 단일 공백으로 이미 축약됨 |
...
expect(normalizeName('정주철 (인)')).toBe('정주철'); // 마커 제거
expect(normalizeName('  Galen   D.  Newman ')).toBe('Galen D. Newman'); // 라틴은 공백만 정리
```

라틴 이름은 어떤 토큰도 한글 단음절이 아니므로 ③ 조건에 걸리지 않고, 공백만 정리되어
`"Galen D. Newman"` 처럼 토큰 사이 단일 스페이스가 유지된다.
...
interface LatinParts {
  ...
  firstGiven: string | null;         // 첫 번째 given 토큰 원문, 예: "Galen" 또는 "G."
...
| --- | --- | --- | --- | --- | --- |
| `Galen D. Newman` | `Newman` | `[Galen, D.]` | `Galen` | `G` | false |
| `G Newman` | `Newman` | `[G]` | `G` | `G` | true |
| `G. Newman` | `Newman` | `[G.]` | `G.` | `G` | true |
| `Seonju Jang` | `Jang` | `[Seonju]` | `Seonju` | `S` | false |
| `CK Kim` | `Kim` | `[CK]` | `CK` | `C` | true |
| `Newman, Galen D.` | `Newman` | `[Galen, D.]` | `Galen` | `G` | false |
| `G N` | `null` | `[G, N]` | `G` | `G` | true |
| `Newman` | `Newman` | `[]` | `null` | `null` | false |

마지막 두 행이 핵심이다.

- `G N` — 마지막 토큰 `N` 이 이니셜이라 **성을 신뢰할 수 없다**(`surname=null`). 그래서 `G N` 은
  닻 없는 bare initials 로 분류되어 누구와도 자동 병합되지 않는다(§5 참조).
- `Newman` — 단일 풀네임 토큰은 **성으로만** 잡힌다(`firstGiven=null`, `firstInitial=null`).
...
| 스크립트 | 키 형식 | 예시 입력 → 키 |
| --- | --- | --- |
| korean | `korean:<공백제거 norm>` | `정 주 철` → `korean:정주철` |
| han | `han:<공백제거 norm>` | `鄭周哲` → `han:鄭周哲` |
| latin (성+이니셜 도출 가능) | `latin:<firstInitial소문자> <surname소문자>` | `Galen D. Newman` → `latin:g newman` |
| latin (성/이니셜 불가) | `latin-raw:<norm소문자>` | `G N` → `latin-raw:g n` |
| mixed/unknown | `raw:<norm소문자>` | `Galen 정` → `raw:galen 정` |
...
`latin:` 키는 *성+첫이니셜*만 쓰므로 `Galen Newman` 과 `Gary Newman` 도 같은 키(`latin:g newman`)에
들어간다. 이는 의도된 동작이다 — 키는 후보를 모으는 단계이고, 실제로 둘이 다른 사람임을
가려내는 일은 `namesMatch` 가 한다(§5의 진리표에서 `Galen Newman` ↔ `Gary Newman` = false).
...
| a | b | 결과 | 적용 규칙 / 이유 |
| --- | --- | :---: | --- |
| `Galen D. Newman` | `G Newman` | **true** | R4 — 같은 성 Newman, 같은 이니셜 G, b가 이니셜 폼 |
| `Seonju Jang` | `S Jang` | **true** | R4 — 같은 성 Jang, 같은 이니셜 S |
| `CK Kim` | `Chang Kim` | **true** | R4 — 같은 성 Kim, 이니셜 C(=CK의 첫 글자) = C(hang) |
| `정 주 철` | `정주철` | **true** | 자간정규화 후 R2 완전일치 |
| `Galen Newman` | `G. Newman` | **true** | R4 — 풀네임 ↔ 자기 이니셜 폼 |
| `Galen Newman` | `Galen Lee` | **false** | R4 실패 — 성이 다름(Newman ≠ Lee) |
| `Galen Newman` | `Gary Newman` | **false** | R5 — 둘 다 풀 given, Galen ≠ Gary |
| `Galen D. Newman` | `Gregory Newman` | **false** | R5 — 둘 다 풀 given, Galen ≠ Gregory |
| `S Jang` | `G Jang` | **false** | R4 실패 — 같은 성이나 이니셜 다름(S ≠ G) |
| `정주철` | `Jucheol Jung` | **false** | R1 — 교차 스크립트(korean vs latin) |
| `G N` | `Galen Newman` | **false** | bare initials — `G N` 은 surname=null (닻 없음) |
...
반대로 `G N` ↔ `Galen Newman` = false 인 이유는 §3.2 에서 본 대로 `G N` 의 마지막 토큰 `N` 이
이니셜이라 `latinParts('G N').surname === null` 이 되고, R4 의 `pa.surname && pb.surname` 조건이
...
성과 첫 이니셜을 모두 도출할 수 있을 때만 `"G Newman"` 형태를 만든다. 둘 중 하나라도 없으면
...
| 입력 | 스크립트 | 점수 | 근거 |
| --- | --- | :---: | --- |
| `Galen Newman` | latin | 3 | 성 Newman + 풀 given Galen |
| `정주철` | korean | 3 | 공백 제거 길이 ≥ 2 |
| `G Newman` | latin | 2 | 성 Newman 있으나 given 이 이니셜뿐 |
| `Newman` | latin | 2 | 성만 있음 |
| `G N` | latin | 1 | 성 null(닻 없음) → ambiguous |
| `정` | korean | 1 | 길이 1 |
| `Galen 정` | mixed | 1 | 기본값 |
...
즉 같은 사람으로 병합된 `Galen Newman`(3)과 `G Newman`(2) 사이에서는 풀네임이 canonical 로
선택된다. 한국어 풀이름(`정주철`, 3)과 단일 글자(`정`, 1)도 마찬가지로 더 완전한 쪽이 이긴다.
...
| --- | --- | --- | --- | --- | --- |
| `0323-000001_[학위논문]_박사학위_영문.pdf` | `0323-000001` | `학위논문` | `박사학위_영문` | `doctoral` | `en` |
| `0323-000001_[대표연구]_journal.png` | `0323-000001` | `대표연구` | `journal` | `null` | `null` |
...
/** "0323-000001 (장선주)" -> { applicantId: "0323-000001", applicantName: "장선주" } */
...
| --- | --- | --- |
| `0323-000001 (장선주)` | `0323-000001` | `장선주` |
| `0323-000001(장선주)` | `0323-000001` | `장선주` (앞 공백 `\s*` 선택적) |
| `0323-000099` | `0323-000099` | `null` (괄호 없음) |
